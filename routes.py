...
def admin_dashboard():
    ...
    sql_warn = text("""
    SELECT s.student_name, s.class_ AS class_name, COUNT(l.id) AS late_count
    FROM laterecords l
    JOIN schoolrecords s ON l.admission_no = s.admission_no
    WHERE l.date >= :month_ago
    GROUP BY l.admission_no, s.student_name, s.class_
    HAVING COUNT(l.id) > 3
...
def export_pdf():
    ...
    for lr in late_records:
        ...
        # sanitize values to avoid None being passed to reportlab
        date_str = str(lr.date) if getattr(lr, "date", None) is not None else ""
        time_str = lr.time.strftime('%H:%M:%S') if getattr(lr, "time", None) else ""
        student_name = lr.student_name or ""
        admission_no = str(lr.admission_no) if getattr(lr, "admission_no", None) is not None else ""
        reason = lr.reason or ""

        p.drawString(30, y, date_str)
        p.drawString(100, y, time_str)
        p.drawString(170, y, student_name)
        p.drawString(350, y, admission_no)
        p.drawString(450, y, reason)
        y -= 20
